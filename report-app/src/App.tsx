...
const TextInput = ({ id, value, onChange, placeholder, type = "text", required = false, hasError = false, inputRef }: any) => (
  <input
    id={id}
    ref={inputRef}
    value={value}
...
const NumberInput = (p: any) => <TextInput {...p} type="number" inputMode="decimal" />;
...
function defaultFormData(): FormData {
  return {
    header: { apparatusType: "", deviceSerial: "", customerOrder: "", customerPO: "", revision: "A", revDate: today() },
...
/* --------- Payload embed (kept from your version) --------- */
  ...
  return (
    <section className="print-page">
      ...
      <div className="print-footer">
        <span><strong>Document No.:</strong> {footer.doc}</span>
        <span><strong>Revision:</strong> {footer.rev}</span>
        <span><strong>Revision Date:</strong> {footer.date}</span>
        ...
        <table className="table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="center">
                <PFN value={data.resultA} onChange={(v) => update("section1.resultA", v)} name="s1a" />
              </td>
              <td className="center">
                <PFN value={data.resultB} onChange={(v) => update("section1.resultB", v)} name="s1b" />
              </td>
              <td className="center">
                <PFN value={data.resultC} onChange={(v) => update("section1.resultC", v)} name="s1c" />
              </td>
            </tr>
          </tbody>
        </table>
...
function TripTable({ section2, update }: { section2: Section2; update: (path: string, v: any) => void }) {
  return (
    <div className="card">
      <h2 className="h1" style={{ fontSize: 18 }}>
            ...
            {tripRows.map(({ key, label }) => {
              ...
              return (
                <tr key={key}>
                  ...
                  <td>
                    <TextInput value={row.settings} onChange={(e: any) => update(`section2.${key}.settings`, e.target.value)} placeholder="e.g., I1 = 4.0 In, tol ±10%" />
                  </td>
                  <td>
                    <TextInput value={row.criteria} onChange={(e: any) => update(`section2.${key}.criteria`, e.target.value)} placeholder="e.g., ≤ 5 sec @ I1" />
                  </td>
                  <td className="center">
                    <TextInput value={row.phaseA} onChange={(e: any) => update(`section2.${key}.phaseA`, e.target.value)} placeholder="value / sec" />
                  </td>
                  <td className="center">
                    <TextInput value={row.phaseB} onChange={(e: any) => update(`section2.${key}.phaseB`, e.target.value)} placeholder="value / sec" />
                  </td>
                  <td className="center">
                    <TextInput value={row.phaseC} onChange={(e: any) => update(`section2.${key}.phaseC`, e.target.value)} placeholder="value / sec" />
                  </td>
                </tr>
              );
            })}
...
export default function App() {
  ...
  const onSavePdf = () => {
    ...
    alert("Tip: In the print dialog, choose 'Save as PDF' and uncheck 'Headers and footers'.");
    window.print();
  };
  ...
  const handlePdfImport = async (file?: File) => {
    ...
    try {
      ...
      if (!jsonStr) {
        alert("No embedded data found. Make sure the PDF was saved from this app.");
        return;
      }
      ...
      <div className="header">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div aria-label="ABB" style={{ fontWeight: 900, fontSize: 26, color: "#e10600", letterSpacing: 1 }}>
              ABB
            </div>
          ...
          <div style={{ textAlign: "right", fontSize: 13 }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center", justifyContent: "flex-end" }}>
              ...
              <SelectInput id="revision" value={data.header.revision} onChange={(e: any) => update("header.revision", e.target.value)} options={REV_LETTERS} />
            </div>
            <div>
        ...
        <div className="grid grid-4" style={{ marginTop: 10 }}>
          ...
          <div>
            ...
            <TextInput id="customerPO" value={data.header.customerPO} onChange={(e: any) => update("header.customerPO", e.target.value)} />
          </div>
        </div>

        <div className="print-hide" style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button className="btn" onClick={onSavePdf}>Save as PDF</button>
          <button className="btn" onClick={onPrint}>Print</button>
          <label className="btn" style={{ cursor: "pointer" }}>
            Resume from PDF
            <input ref={pdfRef} type="file" accept="application/pdf" style={{ display: "none" }} onChange={(e) => handlePdfImport(e.target.files?.[0] ?? undefined)} />
          </label>
          <button className="btn" onClick={reset}>Reset</button>
        </div>
      </div> {/* closes .header */}

      <Section1Table data={data.section1} update={update} vacAInvalid={vacAInvalid} setVacAInvalid={setVacAInvalid} />
    ...
    <>
          ...
          <table className="table">
            ...
            <tbody>
              {data.section7.rows.map((row, idx) => (
                <tr key={idx}>
                  <td>
                    <TextInput value={row.label} onChange={(e: any) => update(`section7.rows.${idx}.label`, e.target.value)} placeholder="Measurement point / notes" />
                  </td>
                  <td>
                    <NumberInput value={row.a ?? ""} onChange={(e: any) => update(`section7.rows.${idx}.a`, toNumber(e.target.value))} />
                  </td>
                  <td>
                    <NumberInput value={row.b ?? ""} onChange={(e: any) => update(`section7.rows.${idx}.b`, toNumber(e.target.value))} />
                  </td>
                  <td>
                    <NumberInput value={row.c ?? ""} onChange={(e: any) => update(`section7.rows.${idx}.c`, toNumber(e.target.value))} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="card" style={{ marginBottom: 40 }}>
        ...
        <p className="muted" style={{ marginTop: 8 }}>
          Procedure Number and Revision: {DOC_NUMBER}, REV {data.header.revision} ({data.header.revDate})
        </p>
      </div>
    </>
  );
